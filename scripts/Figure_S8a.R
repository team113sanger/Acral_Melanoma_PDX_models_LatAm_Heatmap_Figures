...
# Plotting Samples + PDX-X1 ----
# Select Patients that have at least 1 pdx
x1_ids <- metadata |>
  filter(Sample_passage %in% c("Patient", "X1")) |>
  group_by(BR_Sample_ID) |>
  filter(n() > 1)

# Filter alteration list by sample 
x1_compare <- dat |>
  filter(Sample %in% x1_ids$PD_ID)
...
anno_dat <- x1_ids |>
  ...
  select(BR_Sample_ID, BR_ID, passage) |>
...
pdf("figures/Figure_S8a.pdf", height = 8, width = 12)
draw(onco)
dev.off()
